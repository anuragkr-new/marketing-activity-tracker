...
const listSelect = `
  SELECT i.id, i.name, i.theme_id, i.owner_id, i.landing_page_url, i.status, i.created_at,
...
router.post(
  ...
  async (req, res) => {
    ...
    const full = await pool.query(`${listSelect} WHERE i.id = $1`, [
      rows[0].id,
    ]);
...
router.delete(
  ...
  async (req, res) => {
    ...
    await pool.query('DELETE FROM activity_log WHERE initiative_id = $1', [
      id,
    ]);
